...
bool StatementId::operator<(const StatementId& right) const {
  if (number_ != right.number_) return number_ < right.number_;
  return strcmp(str_, right.str_) < 0;
}

Connection::Connection() : db_(NULL), in_memory_(false) {}

Connection::~Connection() { Close(); }

bool Connection::Open(const char* path) { return OpenInternal(path); }
...
void Connection::Close() { CloseInternal(false); }
...
bool Connection::Execute(const char* sql) {
  ...
  if (error != SQLITE_OK) OnSqliteError(error, NULL, sql);
...
StatementRef Connection::GetCachedStatement(const StatementId& id,
                                            const char* sql) {
...
void Connection::OnSqliteError(int rc, Statement* statement,
...
void Connection::CloseInternal(bool forced) {
  ...
  if (SQLITE_OK != rc) {
    OnSqliteError(rc, NULL, "Close Sqlite ");
  }
  db_ = NULL;
}

StatementRef Connection::GetStatementImpl(sql::Connection* tracking_db,
                                          const char* sql) const {
  if (!db_) {
