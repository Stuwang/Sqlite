...
StatementImpl::StatementImpl(Connection* conn, sqlite3_stmt* stmt)
    : stmt_(stmt), connection_(conn) {
  if (connection_) {
...
Statement::Statement() : ref_(), steped_(false), successed_(false) {}

Statement::Statement(const StatementRef& ref)
    : ref_(ref), steped_(false), successed_(false) {}
...
bool Statement::BindCString(int col, const char* value) {
  DCHECK(!steped_);
  return CheckBindOk(
      sqlite3_bind_text(ref_->stmt(), col + 1, value, -1, SQLITE_TRANSIENT));
}
bool Statement::BindString(int col, const std::string& value) {
  DCHECK(!steped_);
  return CheckBindOk(sqlite3_bind_text(ref_->stmt(), col + 1, value.data(),
                                       value.size(), SQLITE_TRANSIENT));
}
bool Statement::BindBlob(int col, const void* blob, int size) {
  DCHECK(!steped_);
  return CheckBindOk(
      sqlite3_bind_blob(ref_->stmt(), col + 1, blob, size, SQLITE_TRANSIENT));
}
...
std::string Statement::ColumnString(int col) const {
  DCHECK(is_valid());
  const char* str =
      reinterpret_cast<const char*>(sqlite3_column_text(ref_->stmt(), col));
  int len = sqlite3_column_bytes(ref_->stmt(), col);

  std::string result;
  if (str && len > 0) result.assign(str, len);
  return result;
}
...
bool Statement::ColumnBlobAsString(int col, std::string* o_res) const {
  ...
  o_res->resize(size);
  if (p && size && o_res->size() != size) return false;
...
bool Statement::ColumnBlobAsVector(int col,
  ...
  return ColumnBlobAsVector(col, reinterpret_cast<std::vector<char>*>(o_res));
}

int Statement::CheckStepError(int err) {
  successed_ = (err == SQLITE_OK || err == SQLITE_ROW || err == SQLITE_DONE);
  steped_ = true;
